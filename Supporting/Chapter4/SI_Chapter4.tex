...
I estimated climate-change sensitivity across terrestrial vertebrates with the CENFA framework using three different resolutions for the species distribution files and the climatic variables:  50 km $\times$ 50 km ,  10 km $\times$ 10 km  and  5 km $\times$ 5 km . The finer the resolution, the better the species' distribution is likely to be captured, particularly for narrow-ranging species (Figure \ref{SI_4_Figure6}). When working with coarser resolutions, the actual geographical distribution of a narrow-ranging species might be overestimated (Figure \ref{SI_4_Figure6}), such that the climatic niche breadth of the species might also be overestimated; consequently, its climate-change sensitivity may be underestimated. However, finer resolutions are more computationally demanding, which can be limiting when working across several thousand species. 
...
\caption[Estimated climate-change sensitivity estimations at three different resolutions]{\textbf{Estimated climate-change sensitivity estimations at three different resolutions (50 km $\times$ 50 km, 10 km$\times$ 10 km and 5 km $\times$ 5 km) against geographical range area (estimated at 1 km$\times$ 1 km).} With the red dashed lines, I highlight the range areas that correspond to the surface area of one grid cell (\textit{i.e.}, 2,500 km$^2$, 100 km$^2$ and 25 km$^2$ respectively). Climate-change sensitivity was estimated using the CENFA framework \citep{Rinnan2019}. I chose to work at a resolution of 5 km $\times$ 5 km and I excluded species whose range area was $\leq$100km$^2$ (blue dashed line), that is, species whose distribution could intersect up to four grid cells at a resolution of 5 km $\times$ 5 km.}
\label{SI_4_Figure7}
\end{figure}
...
%%%%%%%%%%%%%%%%%%%%%%%% Validations -- robustness to using different resolutions

\begin{figure}[h!]
\centering
\includegraphics[scale=0.7, clip]{Supporting/Chapter4/Figures/Minor_corrections_assembled_res/coefficients.pdf}
\caption[Effects for the PGLS models investigating associations between species-level ecological characteristics and climate-change sensitivity, either  using climate-change sensitivity estimated at 5 $\times$ 5 km resolution (y--axis), or using climate-change sensitivity estimated at different resolutions (x--axis; see Methods in main text)]{\textbf{Effects for the PGLS models investigating associations between species-level ecological characteristics and climate-change sensitivity, either  using climate-change sensitivity estimated at 5 $\times$ 5 km resolution (y--axis), or using climate-change sensitivity estimated at different resolutions (x--axis; see Methods in main text)}. Overall, the estimates from both sets of models were congruent. The dashed line is the identity line (y=x).}
\label{SI_4_assembled_coefs}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[scale=0.7, clip]{Supporting/Chapter4/Figures/Minor_corrections_assembled_res/AssembledRes_Categorical.pdf}
\caption[Estimated effects of the categorical traits on climate-change sensitivity, using climate-change sensitivity estimated at different resolutions (see Methods in main text)]{\textbf{Estimated effects of the categorical traits on climate-change sensitivity, using climate-change sensitivity estimated at different resolutions (see Methods in main text; mean effect $\pm$95\% confidence interval).} For each categorical trait, I show the effect size for all levels referring to the reference level (vertical dashed line). \textbf{(a)} For artificial habitat use, the reference level is `Artificial habitat user'; \textbf{(b)} for diel activity, the reference level is `Nocturnal'; \textbf{(c)} for diet, the reference level (for mammals and birds) is `Fruit/nectar' eaters. }
\label{SI_4_assembled_categorical}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[scale=0.7, clip]{Supporting/Chapter4/Figures/Minor_corrections_assembled_res/AssembledRes_Continuous.pdf}
\caption[Effects of the continuous ecological characteristics on climate-change sensitivity, using climate-change sensitivity estimated at different resolutions (see Methods in main text)]{\textbf{Effects of the continuous ecological characteristics on climate-change sensitivity, using climate-change sensitivity estimated at different resolutions (see Methods in main text).} The lines represent the estimated relationships between climate-change sensitivity and ecological characteristics; the shaded areas are 95\% confidence intervals.}
\label{SI_4_assembled_continuous}
\end{figure}
